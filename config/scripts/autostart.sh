...
logger -t "[Auto start]" "正在启动SmartDns"
/usr/bin/smartdns.sh start
fi

if [ $(nvram get caddy_enable) = 1 ] ; then
logger -t "[Auto start]" "正在启动文件管理"
...
logger -t "[Auto start]" "Checking whether the route is connected to the Internet!"
count=0
while :
do
